...
namespace pmc {
    inline uint64_t pmc_hash64(std::string_view key) noexcept {
        ...
        crc ^= 0xFFFFFFFFu;
        return (uint64_t(crc) << 32) | crc;
    }

    inline uint32_t pmc_jump_consistent_hash(uint64_t key, uint32_t buckets) noexcept {
        int64_t b = -1, j = 0;
        while(j < buckets) {
            b = j;
            key = key * 2862933555777941757ULL + 1ULL;
            j = int64_t((double(b) + 1.0) * (double(1ULL << 31) / double((key >> 33) + 1ULL)));
        }
        return uint32_t(b);
    }
    ...
    class ClusterCacheClient {
        public:
            ...
            explicit ClusterCacheClient(ClusterOptions opt)
                : opt_(std::move(opt))
            {
                if(opt_.shardCount == 0) {
                    throw std::invalid_argument("shardCount must be > 0");
                }

                shards_.reserve(opt_.shardCount);
                for(uint32_t i = 0; i < opt_.shardCount; ++i) {
                    CacheClient::Options copt;
                    copt.host = opt_.host;
                    copt.port = uint16_t(opt_.basePort + i);
                    copt.sendTimeout = opt_.sendTimeout;
                    copt.receiveTimeout = opt_.receiveTimeout;

                    shards_.emplace_back(std::move(copt));
                    shards_.back().connect();
                }
            }

            // Shard selection based on consistent hash of key
            uint32_t shardForKey(std::string_view key) const noexcept {
                const uint64_t h = pmc_hash64(key);
                return pmc_jump_consistent_hash(h, opt_.shardCount);
            }

            // Convenience wrappers
            pmc::CacheClient::Response get(std::string_view key) {
                return shard(key).get(key);
            }

            pmc::CacheClient::Response set(std::string_view key, std::string_view val) {
                return shard(key).set(key, val);
            }

            pmc::CacheClient::Response del(std::string_view key) {
                return shard(key).del(key);
            }

            // For pipelining:
            pmc::CacheClient::RequestId enqueueGet(std::string_view key) {
                return shard(key).enqueueGet(key);
            }

            pmc::CacheClient::RequestId enqueueSet(std::string_view key, std::string_view val) {
                return shard(key).enqueueSet(key, val);
            }

            pmc::CacheClient::RequestId enqueueDelete(std::string_view key) {
            ...
            // Flush all shards (optional helper)
            void flushAll() {
                for(auto& c : shards_) c.flush();
            }

        private:
            CacheClient& shard(std::string_view key) noexcept {
                return shards_[ shardForKey(key) ];
            }
